...
export default function AdminUsersPage() {
  ...
  return (
    <div className="space-y-6 pb-14 w-full">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">
              Users & Governance Command Center
            </h1>
            <p className="text-sm text-slate-300 mt-2 leading-relaxed">
              Streamline access control across the hospital hierarchy. Click any category below to switch between all staff, system administrators, investigative committees, executive oversight, and department leadership.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs border border-white/20 transition-all flex items-center gap-2 backdrop-blur-sm shadow-sm"
            >
              <Building2 size={16} className="text-amber-300" />
              Map Department Leaders
            </button>
            <button
              onClick={() => {
                setAssignForm({ employeeId: '', targetRole: 'system_admin', departmentId: '' });
              ...
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold text-xs shadow-lg shadow-indigo-500/25 transition-all flex items-center gap-2"
            >
              <Plus size={16} />
              Configure User Role
            </button>
      ...
      {/* 5 Professional Category Cards (Interactive Tabs stretching full width) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {/* Card 1: All Employees */}
        <div
          onClick={() => setActiveCard('employee')}
          className={`group rounded-2xl p-5 cursor-pointer transition-all duration-200 border-2 ${
            activeCard === 'employee'
              ? 'border-blue-600 bg-blue-50/60 shadow-lg shadow-blue-500/10 scale-[1.01]'
              : 'border-slate-200/80 hover:border-blue-300 bg-white hover:shadow-md'
          }`}
        >
          <div className="flex items-start justify-between">
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-transform group-hover:scale-105 ${
              activeCard === 'employee' ? 'bg-blue-600 text-white shadow-md shadow-blue-500/30' : 'bg-blue-100 text-blue-600'
            }`}>
              <Users size={22} />
            </div>
            {activeCard === 'employee' && (
              <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-blue-700 bg-blue-100/80 px-2 py-0.5 rounded-full border border-blue-300">
                <CheckCircle2 size={11} /> ACTIVE
              </span>
            )}
          </div>
          <div className="mt-3.5">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Staff Directory</p>
            <div className="flex items-baseline gap-2 mt-0.5">
              <h3 className="text-2xl font-black text-slate-900">{isLoadingUsers ? '...' : (usersData?.total || 0)}</h3>
              <span className="text-xs font-semibold text-slate-500">Employees</span>
            </div>
            <p className="text-[11px] text-slate-500 mt-1 line-clamp-1">All registered hospital personnel</p>
          </div>
        </div>

        {/* Card 2: System Admins */}
        <div
          onClick={() => setActiveCard('system_admin')}
          className={`group rounded-2xl p-5 cursor-pointer transition-all duration-200 border-2 ${
            activeCard === 'system_admin'
              ? 'border-rose-600 bg-rose-50/60 shadow-lg shadow-rose-500/10 scale-[1.01]'
              : 'border-slate-200/80 hover:border-rose-300 bg-white hover:shadow-md'
          }`}
        >
          <div className="flex items-start justify-between">
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-transform group-hover:scale-105 ${
              activeCard === 'system_admin' ? 'bg-rose-600 text-white shadow-md shadow-rose-500/30' : 'bg-rose-100 text-rose-600'
            }`}>
              <Lock size={22} />
            </div>
            {activeCard === 'system_admin' && (
              <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-rose-700 bg-rose-100/80 px-2 py-0.5 rounded-full border border-rose-300">
                <CheckCircle2 size={11} /> ACTIVE
              </span>
            )}
          </div>
          <div className="mt-3.5">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">System Admins</p>
            <div className="flex items-baseline gap-2 mt-0.5">
              <h3 className="text-2xl font-black text-slate-900">{isLoadingAdmins ? '...' : systemAdmins.length}</h3>
              <span className="text-xs font-semibold text-slate-500">Admins</span>
            </div>
            <p className="text-[11px] text-slate-500 mt-1 line-clamp-1">Full portal configuration rights</p>
          </div>
        </div>

        {/* Card 3: IMC Committee */}
        <div
          onClick={() => setActiveCard('imc')}
          className={`group rounded-2xl p-5 cursor-pointer transition-all duration-200 border-2 ${
            activeCard === 'imc'
              ? 'border-indigo-600 bg-indigo-50/60 shadow-lg shadow-indigo-500/10 scale-[1.01]'
              : 'border-slate-200/80 hover:border-indigo-300 bg-white hover:shadow-md'
          }`}
        >
          <div className="flex items-start justify-between">
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-transform group-hover:scale-105 ${
              activeCard === 'imc' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/30' : 'bg-indigo-100 text-indigo-600'
            }`}>
              <ShieldCheck size={22} />
            </div>
            {activeCard === 'imc' && (
              <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-indigo-700 bg-indigo-100/80 px-2 py-0.5 rounded-full border border-indigo-300">
                <CheckCircle2 size={11} /> ACTIVE
              </span>
            )}
          </div>
          <div className="mt-3.5">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">IMC Committee</p>
            <div className="flex items-baseline gap-2 mt-0.5">
              <h3 className="text-2xl font-black text-slate-900">{isLoadingImc ? '...' : imcMembers.length}</h3>
              <span className="text-xs font-semibold text-slate-500">Members</span>
            </div>
            <p className="text-[11px] text-slate-500 mt-1 line-clamp-1">Quality & investigation team</p>
          </div>
        </div>

        {/* Card 4: Management */}
        <div
          onClick={() => setActiveCard('management')}
          className={`group rounded-2xl p-5 cursor-pointer transition-all duration-200 border-2 ${
            activeCard === 'management'
              ? 'border-purple-600 bg-purple-50/60 shadow-lg shadow-purple-500/10 scale-[1.01]'
              : 'border-slate-200/80 hover:border-purple-300 bg-white hover:shadow-md'
          }`}
        >
          <div className="flex items-start justify-between">
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-transform group-hover:scale-105 ${
              activeCard === 'management' ? 'bg-purple-600 text-white shadow-md shadow-purple-500/30' : 'bg-purple-100 text-purple-600'
            }`}>
              <Award size={22} />
            </div>
            {activeCard === 'management' && (
              <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-purple-700 bg-purple-100/80 px-2 py-0.5 rounded-full border border-purple-300">
                <CheckCircle2 size={11} /> ACTIVE
              </span>
            )}
          </div>
          <div className="mt-3.5">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Management Team</p>
            <div className="flex items-baseline gap-2 mt-0.5">
              <h3 className="text-2xl font-black text-slate-900">{isLoadingMgmt ? '...' : managementMembers.length}</h3>
              <span className="text-xs font-semibold text-slate-500">Executives</span>
            </div>
            <p className="text-[11px] text-slate-500 mt-1 line-clamp-1">Executive leadership & COO</p>
          </div>
        </div>

        {/* Card 5: Leadership Mapping */}
        <div
          onClick={() => setActiveCard('mapping')}
          className={`group rounded-2xl p-5 cursor-pointer transition-all duration-200 border-2 ${
            activeCard === 'mapping'
              ? 'border-amber-600 bg-amber-50/60 shadow-lg shadow-amber-500/10 scale-[1.01]'
              : 'border-slate-200/80 hover:border-amber-300 bg-white hover:shadow-md'
          }`}
        >
          <div className="flex items-start justify-between">
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-transform group-hover:scale-105 ${
              activeCard === 'mapping' ? 'bg-amber-600 text-white shadow-md shadow-amber-500/30' : 'bg-amber-100 text-amber-700'
            }`}>
              <Building2 size={22} />
            </div>
            {activeCard === 'mapping' && (
              <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-amber-800 bg-amber-100/80 px-2 py-0.5 rounded-full border border-amber-300">
                <CheckCircle2 size={11} /> ACTIVE
              </span>
            )}
          </div>
          <div className="mt-3.5">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Dept Mapping</p>
            <div className="flex items-baseline gap-2 mt-0.5">
              <h3 className="text-2xl font-black text-slate-900">{isLoadingDepts ? '...' : departments.length}</h3>
              <span className="text-xs font-semibold text-slate-500">Depts</span>
            </div>
            <p className="text-[11px] text-slate-500 mt-1 line-clamp-1">HOD / Incharge / COO Matrix</p>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-2xl bg-white border border-slate-200/80 shadow-md p-6 sm:p-8 animate-in fade-in duration-200">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 border-b border-slate-100 pb-6">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md shadow-rose-500/20 transition-all"
            >
              <Plus size={16} /> Assign System Administrator
            </button>
          </div>
          ...
          {isLoadingAdmins ? (
            ...
            <div className="py-16 text-center text-slate-400 text-sm font-medium bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
              No System Administrators found. Check backend server connection.
            </div>
          ) : (
        ...
        <div className="rounded-2xl bg-white border border-slate-200/80 shadow-md p-6 sm:p-8 animate-in fade-in duration-200">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 border-b border-slate-100 pb-6">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md shadow-indigo-500/20 transition-all"
            >
              <Plus size={16} /> Add IMC Committee Member
            </button>
          </div>
          ...
          {isLoadingImc ? (
            ...
            <div className="py-16 text-center text-slate-400 text-sm font-medium bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
              No personnel currently assigned to the IMC Committee.
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {imcMembers.map(m => (
        ...
        <div className="rounded-2xl bg-white border border-slate-200/80 shadow-md p-6 sm:p-8 animate-in fade-in duration-200">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 border-b border-slate-100 pb-6">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md shadow-purple-500/20 transition-all"
            >
              <Plus size={16} /> Add Management Executive
            </button>
          </div>
          ...
          {isLoadingMgmt ? (
            ...
            <div className="py-16 text-center text-slate-400 text-sm font-medium bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
              No executive personnel currently assigned to Management.
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <div className="rounded-2xl bg-white border border-slate-200/80 shadow-md p-6 sm:p-8 animate-in fade-in duration-200">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 border-b border-slate-100 pb-6">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md shadow-amber-500/20 transition-all"
            >
              <Plus size={16} /> Assign Department Leader
            </button>
          </div>
              ...
              {[
                ...
                >
                  <div className={`w-4 h-4 rounded-full mt-0.5 border-2 flex items-center justify-center ${
                    assignForm.targetRole === opt.id ? 'border-blue-600 bg-blue-600' : 'border-slate-300'
                  }`}>
